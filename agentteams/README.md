...
2026-09-20 补充：数据库底座已在 PHX 边缘算力节点（129.146.24.141 / 100.95.247.115）拉起专属 PostgreSQL 14 实例（端口 5433），全量承接 300 门店、1500 台冷柜、5100 批次商品与 43.2 万条完整高频物理遥测数据（查询耗时 0.059ms）。两项条件测试支持直连该实例 2/2 实跑通过（零 skip）。常规断网全量测试继续保留两项 skip，详见唯一[待办](../docs/待办.md)。
